...
function Confirm() {
  ...
  },
  {
    ...
    returns: [
      ...
      { name: 'zoomFactor', type: 'number', desc: 'Current zoom, from 0 to maxZoomFactor as expo-camera defines it.' },
      { name: 'maxZoomFactor', type: 'number', desc: 'Upper bound this hook allows.' },
      { name: 'flashMode', type: "'auto' | 'on' | 'off'", desc: 'Flash behaviour for the next capture.' },
      { name: 'hasPermission', type: 'boolean', desc: 'Whether camera permission has been granted.' },
      { name: 'selectedLook', type: 'CameraLook', desc: 'Label only. Looks are a Pixel Camera app feature and are not applied by this hook.' },
      { name: 'isUltraLowLightVideoActive', type: 'boolean', desc: 'Interface flag only; the underlying mode is not third-party controllable.' },
    ],
    actions: [
      { name: 'toggleFacing()', type: '() => void', desc: 'Switches between the front and rear camera.' },
      { name: 'setZoom(value)', type: '(value: number) => void', desc: 'Sets the zoom ratio, clamped to maxZoomFactor.' },
      { name: 'setFlash(mode)', type: "(mode: 'auto' | 'on' | 'off') => void", desc: 'Chooses flash behaviour.' },
      { name: 'setLook(look)', type: '(look: CameraLook) => void', desc: 'Stores a Look label for your own interface.' },
    ],
