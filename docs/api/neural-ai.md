...
**Build note:** genai-prompt beta4 is compiled with Kotlin 2.3 while Expo 57 builds with Kotlin 2.1.20. The module passes `-Xskip-metadata-version-check` for its own compile and pins every `kotlin-stdlib` artifact to the project's Kotlin version (see `modules/pixel-nano/android/build.gradle`).
...
### Example
```tsx
import { useGeminiNano, useSpeech, HapticButton } from './src';
