...
4. **Pin compileSdk 36 / targetSdk 36 / minSdk 26** with `expo-build-properties` (37 is blocked by AGP 8.12 vs `android-37.0`, see deep dive); declare `android.hardware.neural_processing_unit` (`required=false`) and `ACCESS_LOCAL_NETWORK` where relevant in `app.json`.
