...
> Full telemetry and observability pass on the physical Pixel 11 Pro (grizzly, Android 17 / SDK 37) over wireless adb, dev-client build with the `PixelNative` module. Goal: **no mocked connections**. Each hook is verified from two sides where possible: the app's own provenance log (`[PixelForge]` in `adb logcat -s ReactNativeJS`) and an independent system view (`dumpsys`, HAL logs).
...
| `useAudio` | **HW** | `dumpsys audio`: `rec start … src:VOICE_RECOGNITION pack:com.pixelforge.sdk`, client format 1ch 16 kHz PCM; meter read −46 dBFS in a quiet room |
...
| `useDisplay` keep-awake | **HW** | `SCREEN_BRIGHT_WAKE_LOCK` with `WorkSource{10398 com.pixelforge.sdk}` |
