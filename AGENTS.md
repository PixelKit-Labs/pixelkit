...
3. **No mocks.** Every hook exposes `source: 'hardware' | 'derived' | 'simulated' | 'unavailable'` (`src/core/observability.ts`). Never substitute a plausible default for a value that could not be read; render `null` as "—" and pass `source` to `MetricCard`. Only NFC, BLE and UWB are currently simulated, and every surface that shows them says so. HiLight is real only when `useHiLight().availability` is `shizuku` (Shizuku shell helper, `modules/pixel-hilight`); otherwise it is a labelled on-screen mirror.
